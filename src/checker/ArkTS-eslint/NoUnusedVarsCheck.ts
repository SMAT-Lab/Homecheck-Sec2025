...
import { ArkMethod, ClassSignature, MethodSignature, Stmt, ArkAssignStmt, Local, ArkInvokeStmt,
  ts, AstTreeUtils, ArkFile, ArkNamespace, Value, FieldSignature, AbstractBinopExpr,
  NamespaceSignature, ArkReturnStmt, AbstractInvokeExpr, ArkIfStmt, FunctionType,
  ArkInstanceInvokeExpr, ExportInfo, ClassType, FullPosition, LineColPosition,
  ArkThrowStmt
...
type Var_base = { name?: string; type?: { name?: string }; usedStmts?: Stmt[]; declaringStmt?: Stmt; };
const leftNameReg = /\[|\]/g;
...
type Method_Param = { arrayElements?: { name?: string }[]; name: string; objElements: []; };
...
const importNoused = this.noUsedImport(astRoot);
...
const noUsedtype = this.noUsedType(astRoot); //包含interface便于处理
...
const generics = this.getUnusedGenerics(astRoot);
...
nousedSet = nousedSet.filter(item =>
!(item.local === 'all' &&
  inusedGlobal.has(item.name?.toString?.()) && inusedGlobal.get(item.name?.toString?.()) !== item?.declareStmt)
);
//过滤特殊数组
nousedSet = nousedSet.filter(item =>
!(item.local === 'all' &&
  inNeedSpecialArray.has(item.name?.toString?.()) && inNeedSpecialArray.get(item.name?.toString?.()) === null)
);
nousedSet = nousedSet.filter(item =>
!(item.local === 'local' &&
  inNeedSpecialArray.has(item.name?.toString?.()) && inNeedSpecialArray.get(item.name?.toString?.())?.stmt === item.stmt)
);
...
private pushAllVar(stmt: Stmt, allVar: noUseVars[], nousedSet: noUseVars[], filePath: string): void {
...
return match[1].split(/[,\s]+/).map(v => v.trim()).filter(Boolean);      
}

private pushAllGlobal(commentCode: string, comment: string[], posion: FullPosition, allVar: noUseVars[], 
nousedSet: noUseVars[], filePath: string): void {
for (const conString of comment) {
const posionIntext = NoUnusedVarsCheckUtils.getTextPosition(commentCode, conString);
...
private mergeCaughtErrors(options: Options, mergedOptions: TranslatedOptions): void {
...
private mergeDestructuredArray(options: Options, mergedOptions: TranslatedOptions): void {
if (typeof options === 'object' && options?.destructuredArrayIgnorePattern) {
  mergedOptions.destructuredArrayIgnorePattern = new RegExp(options.destructuredArrayIgnorePattern);
}
}

private mergeBooleanFlags(options: Options, mergedOptions: TranslatedOptions): void {
if ( typeof options === 'object' && options?.ignoreClassWithStaticInitBlock !== undefined) {
mergedOptions.ignoreClassWithStaticInitBlock =
  options.ignoreClassWithStaticInitBlock;
}
if ( typeof options === 'object' && options?.ignoreRestSiblings !== undefined) {
mergedOptions.ignoreRestSiblings = options.ignoreRestSiblings;
}
if (typeof options === 'object' && options?.reportUsedIgnorePattern !== undefined) {
...
private getNoUsedVars( arkfile: ArkFile, exportNames: string[], methods: ArkMethod[], mergedOptions: TranslatedOptions,
nameSpaceTrue: boolean, parenttFtrue: boolean, allvar: noUseVars[], usedAllGlobalsKeys: Map<string, Stmt | null>,
specialArray: Map<string, {stmt: Stmt} | null>): noUseVars[] {
let nousedSet: noUseVars[] = [];
//存放全局被使用变量最后进行筛选(最优解)
...
nousedSet = this.processBody(methods, exportNames, mergedOptions, nousedSet, usedAllGlobalsKeys, 
parenttFtrue, usedArrayPartnGlobalsKeys, allvar, specialArray);

return nousedSet;
}
private processMethod(methods: ArkMethod[], arkfile: ArkFile, nameSpaceTrue: boolean, nousedSet: noUseVars[]): noUseVars[] {
...
const posion = NoUnusedVarsCheckUtils.getAccuratePosition(methodCode, methodName);
const lineNo = posion.line + lineNum[0] - 1;
let colNo = posion.col;
if (methodCode.startsWith(' ')) {
  colNo = col + colNo;
}
const methodSign = method.getSignature();
const exitMethod = this.methodOrClassUsed(arkfile, methodSign, false);
// 判断是否是全局函数支持 export
...
if (!(method?.isExport() || (isDefaultClass && isExport)) && !exitMethod && !nameSpaceTrue) {
  nousedSet.push({varType: VarType.Method, stmt: method.getBody()?.getCfg().getStmts()[0] ?? ({} as Stmt),
...
private processBody(methods: ArkMethod[], exportNames: string[], mergedOptions: TranslatedOptions, nousedSet: noUseVars[],
usedAllGlobalsKeys: Map<string, Stmt | null>,
parenttFtrue: boolean,
usedArrayPartnGlobalsKeys: string[],
allvar: noUseVars[], specialArray: Map<string, {stmt:Stmt} | null>
): noUseVars[] {
for (const method of methods) {
...
const arrayBindingPatternNames = arrayBindingPattern.flatMap((param, paramIndex) => param.arrayElements
      ?.map((element, elementIndex) => ({ name: element.name, methodParamIndex: paramIndex, elementIndex: elementIndex,
      })).filter((item) => Boolean(item.name)) || []
);
const arrayElementNames = [...objArrayBindingPatternNames, ...arrayBindingPatternNames];
...
this.execLocalandGlobal(method, usedGlobals, globalKeys, nousedSet, arrayElementNames, methodStmts,
...
private commentGlobalVars(usedGlobals: Map<string, Value>, nousedSet: noUseVars[]): void {
     nousedSet.filter(item => {!(item.isAll && usedGlobals.has(item.name.toString()))});
}
private extractObjectArrayBindingPatternNames(method: ArkMethod): { name: string; methodParamIndex: number; elementIndex: number }[] {
return method.getParameters().flatMap((param, paramIndex) => this.extractObjectArrayPatterns( method.getCode()?.toString() ?? ''
).flatMap((objParam) => objParam.arrayElements?.map((element, elementIndex) => ({
...
private execLocalandGlobal(method: ArkMethod, usedGlobals: Map<string, Value>, globalKeys: string[],
nousedSet: noUseVars[],
arrayElementNames: {
name?: string;
...
this.processLocalVar(varLocal, method, usedGlobals, globalKeys, nousedSet, usedArrayPartnGlobalsKeys,
parenttFtrue,
exportNames,
mergedOptions, specialArray
);
...
return (method.getParameters() as unknown as Method_Param[]).filter( (param) => param.name === 'ArrayBindingPattern');
}

private getObjectBindingPatternParameters(method: ArkMethod): Method_Param[] {
return (method.getParameters() as unknown as Method_Param[]).filter((param) => param.name === 'ObjectBindingPattern');
}

private processArrayBindingPatternVars(
stmt: ArkAssignStmt, arrayElementNames: {name?: string; methodParamIndex: number; elementIndex: number;}[],
globalKeys: string[], method: ArkMethod, nousedSet: noUseVars[]
) {
...
nousedSet.push({varType: VarType.ArgsP, stmt: stmt, name: unusedParam.name, declareStmt: this.isMyselfDec(stmt),
  isMethodParam: true,
...
nousedSet.push({varType: VarType.ArgsP, stmt: stmt, name: unusedParam.name, declareStmt: this.isMyselfDec(stmt),
  isMethodParam: true, paramIndex: unusedParam.methodParamIndex, arkMethod: method, methodName: method.getName(),
  argPosion: position, filePath: filePath,
});
}
}

private processRegularVars(stmt: ArkAssignStmt, globalKeys: string[], method: ArkMethod, 
nousedSet: noUseVars[], usedGlobals: Map<string, Value>, allVar: noUseVars[]
): void {
...
if (globalKeys.includes(left.name) && method.getName() !== '%dflt' && allVar.find(item => item.name === left.name)) {
this.addallVarByEndIndex(stmt, left.name, nousedSet);
}
}
private addallVarByEndIndex(stmt: Stmt, name: string, nousedSet: noUseVars[]): void {
const result = nousedSet.find((item) => item.name === name && item.varType === VarType.Var && item.local === 'all'); 
if (result) {
//修改nousedSet 中的
result.stmt = stmt;
...
private computeVariablePosition(method: ArkMethod, varName: string, methodParamIndex: number): [number, number, number] {
const lineNo = method.getLine() ?? -1;
const methodCol = method.getColumn() ?? -1;
const posion = NoUnusedVarsCheckUtils.getTextPosition(method.getCode() ?? '', varName);
...
methodStmts: Stmt[], arrayElementNames: { name?: string; methodParamIndex: number; elementIndex: number;}[],
globalKeys: string[], method: ArkMethod, nousedSet: noUseVars[], usedGlobals: Map<string, Value>, allVar: noUseVars[],
) {
...
//扩展变量为箭头函数
if ((localType?.methodSignature && !localType.methodSignature.getMethodSubSignature().getMethodName().startsWith('%')) || key === 'this') {
  continue;
}
...
const paramDefs = NoUnusedVarsCheckUtils.extractParameters(method.getCode() ?? '');
if (
!key.includes('%dflt') &&
key !== 'ArrayBindingPattern' &&
key !== 'ObjectBindingPattern' &&
...
if (stmt instanceof ArkAssignStmt) {
  ...
  if (
    leftName && !isModiferStart &&
    !(
      (this.isVarUsed(left, true, usedGlobals, method) ||
      this.isTextUsed(bodyCode, leftName)) 
    )
  ) {
...
this.addNoUseVar(leftName, stmt, 'local', true, false, true, nousedSet);
return true;
...
const isallVar = method.getDeclaringArkClass().getName() === '%dflt' && method.getName() === '%dflt';
this.addNoUseVar(
  leftName,
  stmt,
  isallVar ? 'all' : 'local',
...
return !methodName.startsWith('%') && this.execMethodStmt(stmt, left, methodName);
});//本层级
// 如果没有返回 true，表示没有使用
...
const originalText = stmt.getOriginalText() ?? '';
if (this.isTextUsed(originalText, left.name ?? 'errorP')) {
  return true;
}
return stmt
  .getInvokeExpr()
...
const exportinfo = arkfile.getExportInfos();
const exportNames =
exportinfo.map((info) => {
  return info.getNameBeforeAs() || info.getOriginName();
}) ?? [];
if (isClass && signature instanceof ClassSignature) {
const className = signature.getClassName() ?? '';
return this.handleClass(arkfile, signature, exportNames, className);
} else if (signature instanceof MethodSignature) {
return this.handleStmts(arkfile, signature, isClass);
}
return false;
}
...
for (const clas of arkfile.getClasses() ?? []) {
if (clas.getSignature() === signature) {
  continue;
}
...
if (
  ...
  this.handleStmts(arkfile, signature, true)
) {
  return true;
}
//处理 new Object()情况
...
for (const item of fields) {
  ...
  if (this.isTextUsed(ui, className) && !this.isTextUsed(ui, `$${className}`)) {
    return true;
  }
}
}
...
if (
  stmt instanceof ArkAssignStmt &&
  stmt.getLeftOp().toString() === 'this'
) {
  continue;
}
if (
  isClass &&
  signature instanceof ClassSignature &&
  this.isClassAssigned(stmt, signature)
) {
  return true;
...
return ((leftopType instanceof ClassType && leftopType.getClassSignature() === signature) || this.isTextUsed(tighttype, classname) || 
isLeftOpUsed || (leftopType.toString() === classname)) && (right.toString() !== 'this' && !this.isTextUsed(originalText, 'this'));
}
...
if (stmt instanceof ArkAssignStmt) {
  ...
  if (this.isTextUsed(leftCode, classname)) {
    return true;
  }
}
}
...
if (stmt instanceof ArkThrowStmt && stmt.getOp() instanceof AbstractInvokeExpr) {
...
//判断执行函数内参数是定义函数 坑爹
private callFunctionDecParams(invokeExpr: AbstractInvokeExpr | null, signature: MethodSignature | ClassSignature): boolean {
if (!invokeExpr) {
return false;
}
...
const rightOpInvoke = (arg as unknown as {declaringStmt: ArkAssignStmt})?.declaringStmt?.getRightOp();
const invokebaseType = (rightOpInvoke instanceof ArkInstanceInvokeExpr) ? rightOpInvoke?.getBase()?.getType() : type;
return invokebaseType instanceof FunctionType && invokebaseType.getMethodSignature() === signature;
});
}
...
unuseds = unuseds.filter((item) => !(item.varType === VarType.ArgsP && mergedOptions.destructuredArrayIgnorePattern?.test(
        item.name.toString()
      )
    )
);
...
private setWarnInfoForVarTypePosion(noUseVar: noUseVars, warnInfo: any): void {
switch (noUseVar.varType) {
case VarType.Var:
...
private createDefect(warnInfo: any, noUseVar: noUseVars, severity: number): Defects {
return new Defects(warnInfo.line, warnInfo.startCol, warnInfo.endCol, this.addDescription(noUseVar), severity, this.rule.ruleId,
warnInfo.filePath,
...
private getLineAndColumn(noUseVar: noUseVars): {line: number; startCol: number; endCol: number; filePath: string;} {
if (!noUseVar.stmt || typeof noUseVar.stmt.getOriginPositionInfo !== 'function') {
...
const arkFile = noUseVar.stmt.getCfg()?.getDeclaringMethod()?.getDeclaringArkFile();
...
const posions = NoUnusedVarsCheckUtils.getTextPosition(finallyOrigText, noUseVar.name.toString());

if (startCol === -1 || !arkFile) {
return { line, startCol: -1, endCol: -1, filePath: '' };
} 
line = posions.line + line; // 行号从0开始，所以加1 
startCol = (posions.line === 0 ? posions.column + startCol : posions.column + 1) - 1; // 列号从1开始，所以-1
const endCol = startCol + noUseVar.name.toString().length - 1;
return {line, startCol, endCol, filePath: arkFile.getFilePath()};
}

private getLineAndColumn_line(originPosition: LineColPosition | undefined, declarePosition: LineColPosition | undefined, 
arkMethod: ArkMethod | undefined): number {
let line = originPosition && (originPosition?.getLineNo() ?? -1) >= 0 ? originPosition.getLineNo() : arkMethod?.getLine() ?? -1;
...
const arkMethod = noUseVar.arkMethod;
const originText = arkMethod ? arkMethod.getCode() ?? '' : noUseVar.stmt?.getOriginalText() ?? '';
const textOrigtext = arkMethod ? arkMethod.getCode() ?? '' : originText;
const finallyOrigText = textOrigtext === '' ? noUseVar?.declareStmt?.getOriginalText() ?? '' : textOrigtext;
return finallyOrigText;
}
private getLineAndColumn_startLine(originPosition: LineColPosition | undefined, declarePosition: LineColPosition | undefined, 
arkMethod: ArkMethod | undefined): number {
let startCol = originPosition && (originPosition?.getLineNo() ?? -1) >= 0 ? originPosition.getColNo() : arkMethod?.getColumn() ?? -1;
...
private execLoopParams(cd: ts.NodeArray<ts.ParameterDeclaration>, params: string[], node: ts.ArrowFunction, usedGlobalP: string[]): void {
...
private exceLoopParamsStatement(statement: ts.Statement, params: string[], usedGlobalP: string[] ): void {
...
private exceLoopParams_next(cd: ts.NodeArray<ts.ParameterDeclaration>, params: string[]): string[] {
  for (const cdd of cd) {
...
private processDefaultImport(defaultImportName: string, source: string, importMap: Map<string, { used: boolean; source: string }>): void {
importMap.set(defaultImportName, {used: false, source: source});
}

private processNamedBindings(namedBindings: ts.ImportClause['namedBindings'], source: string, importMap: Map<string, { used: boolean; source: string }>
): void {
...
private processNamespaceImport(
...
// 主函数：检测未使用的导入
private noUsedImport(astRoot: ts.SourceFile): string[] {
...
typeMap.set(node.name.text, {used: isExported ? true : false, kind: 'type别名', pos: node.name.getStart(astRoot)});
} else if (ts.isInterfaceDeclaration(node)) {
const isExported = node.modifiers && node.modifiers.some((mod) => mod.kind === ts.SyntaxKind.ExportKeyword);
typeMap.set(node.name.text, {used: isExported ? true : false, kind: '接口', pos: node.name.getStart(astRoot)});
}
ts.forEachChild(node, (child) =>
...
private noUsedType(astRoot: ts.SourceFile): astType[] {
const typeMap = new Map<string, { used: boolean; kind: string; pos: number }>();
const noUsedTypes: { name: string; kind: string; line: number; character: number;}[] = [];
...
if (!info.used) {
  ...
  noUsedTypes.push({name, kind: info.kind, line: line + 1, character: character + 1});
}
}
return noUsedTypes;
}
...
private isTextUsed(code: string, name: string): boolean {
// 转义正则特殊字符
...
private collectGenerics(
node: ts.Node,
typeMap: Map<string, { used: boolean; kind: string; pos: number, node: ts.Node }>
): void {
...
ts.isFunctionExpression(node) || ts.isInterfaceDeclaration(node) || 
ts.isClassDeclaration(node) || ts.isArrowFunction(node) || ts.isMethodDeclaration(node) || 
ts.isClassExpression(node) || ts.isFunctionTypeNode(node) ) {
this.collectTypeAliasGenerics(node, typeMap);
}

if ((ts.isTypeAliasDeclaration(node) && ts.isConstructorTypeNode(node.type)) || ts.isConstructSignatureDeclaration(node)) {
this.collectConstructorTypeGenerics(node, typeMap);
}
}

// Collect generics for type aliases and function declarations
private collectTypeAliasGenerics(
node: | ts.TypeAliasDeclaration | ts.FunctionDeclaration | ts.FunctionExpression |
ts.InterfaceDeclaration | ts.ClassDeclaration | ts.ArrowFunction | ts.MethodDeclaration |
ts.FunctionTypeNode | ts.ClassExpression,
typeMap: Map<string, { used: boolean; kind: string; pos: number, node: ts.Node }>
): void {
...
node.typeParameters.forEach((typeParam) => {
  ...
  if (!typeMap.has(genericKey)) {
    typeMap.set(genericKey, {used: false, kind: typeParam.name.text, pos: typeParam.getStart(), node: node});
  }
});
}
}
...
private collectConstructorTypeGenerics(
...
private checkGenericUsages(typeMap: Map<string, {used: boolean; kind: string; pos: number, node: ts.Node}>): void {
...
private isGenericUsedInType(node: ts.Node): string | null {
if (!node) {
return null;
}
...
private handleTypeReferenceNode(node: ts.TypeReferenceNode): string | null {
...
private handleConditionalTypeNode(
node: ts.ConditionalTypeNode
): string | null {
return (this.isGenericUsedInType(node.checkType) || this.isGenericUsedInType(node.extendsType) || this.isGenericUsedInType(node.trueType) ||
this.isGenericUsedInType(node.falseType)
);
}

private handleIndexedAccessTypeNode(
node: ts.IndexedAccessTypeNode
): string | null {
return (this.isGenericUsedInType(node.objectType) || this.isGenericUsedInType(node.indexType) );
}

private handleFunctionTypeNode(
...
private checkChildrenForGeneric(node: ts.Node): string | null {
...
private getUnusedGenerics(astRoot: ts.SourceFile): astType[] {
const typeMap = new Map<string, { used: boolean; kind: string; pos: number; node: ts.Node}>();
const unusedGenerics: { name: string; kind: string; line: number; character: number;}[] = [];
...
if (!info.used) {
  ...
  unusedGenerics.push({name: info.kind, kind: info.kind, line: line + 1, character: character + 1});
}
}
...
// 递归遍历 AST
private traverseAST(node: ts.Node, namespaceName: string): boolean {
...
if ( propertyAccess.expression.kind === ts.SyntaxKind.Identifier && (propertyAccess.expression as ts.Identifier).text === namespaceName) {
  // 如果访问的属性（函数或变量）也匹配
  return true;
}
}
...
if ( qualifiedName.left.kind === ts.SyntaxKind.Identifier && (qualifiedName.left as ts.Identifier).text === namespaceName) {
...
private isMethodUsingNamespace(
method: ArkMethod,
namespaceSignature: NamespaceSignature
): boolean {
const stmts = method.getBody()?.getCfg()?.getStmts() ?? [];
for (const stmt of stmts) {
if (stmt instanceof ArkInvokeStmt && this.isInvokeUsingNamespace(stmt, namespaceSignature)) {
  return true;
}
}
return false;
...
private isInvokeUsingNamespace(
stmt: ArkInvokeStmt,
namespaceSignature: NamespaceSignature
): boolean {
const invokeExpr = stmt.getInvokeExpr();
const methodSignature = invokeExpr.getMethodSignature();
if (methodSignature instanceof NamespaceSignature && methodSignature === namespaceSignature) {
return true;
}
...
private isArgumentUsingNamespace( arg: any, namespaceSignature: NamespaceSignature): boolean {
...
private collectUnusedClasses(targetFile: ArkFile, exportNames: string[], mergedOptions: TranslatedOptions,
allvar: noUseVars[], inusedGlobal: Map<string, Stmt | null>, specialArray: Map<string, {stmt:Stmt} | null>
): noUseVars[] {
let nousedSet: noUseVars[] = [];
const arkFileAllClass = NoUnusedVarsCheckUtils.getArkFileAllClasss(targetFile);
...
const isUsed = this.methodOrClassUsed(targetFile, targetClass.getSignature(), true);
...
if (!(isExported || (!isDefault && isUsed)) && !isInternal && extendedClass.size === 0) {
  if (methodStatic) {
    nousedSet.push({varType: VarType.Class, arkClass: targetClass, name: className, static: true});
  } else {
    nousedSet.push({varType: VarType.Class, arkClass: targetClass, name: className });
  }
}
nousedSet = nousedSet.concat(this.getNoUsedVars(targetFile, exportNames, methods, mergedOptions, false, false, allvar, inusedGlobal, specialArray));
}
return nousedSet;
}

private collectUnusednameSpaces(namespaces: ArkNamespace[], targetFile: ArkFile, exportNames: string[],
mergedOptions: TranslatedOptions, allvar: noUseVars[], inusedGlobal: Map<string, Stmt | null>, 
specialArray: Map<string, {stmt:Stmt} | null>
): noUseVars[] {
let nousedSet: noUseVars[] = [];
...
let currentNamespace: ArkNamespace | undefined | null = namespace.getDeclaringArkNamespace();
...
const ismodule = this.isTextUsed(namespaceCode.substring(0, 20), 'module');
if (
  !(namespaceName !== '' && currentNamespaceTrue) && !namespace.isExport() &&
  !this.isNamespaceUse(targetFile, namespace.getSignature()) && !ismodule &&
  !this.isTextUsed(namespaceCode, 'global') && !exportNames.includes(nameSpaceName) ) {
  nousedSet.push({varType: VarType.Class, arkNamespace: namespace, name: nameSpaceName
  });
}
nousedSet = nousedSet.concat(
  this.getNoUsedVars(targetFile, exportNames, methods, mergedOptions, nameSpaceTrue, currentNamespaceTrue || nameSpaceTrue,
    allvar, inusedGlobal, specialArray
  )
);
}
return nousedSet;
