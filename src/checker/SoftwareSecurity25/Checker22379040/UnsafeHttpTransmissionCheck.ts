...
const logger = Logger.getLogger(LOG_MODULE_TYPE.HOMECHECK, 'UnsafeHttpTransmissionCheck');

const gMetaData: BaseMetaData = {
    severity: 1,
    ruleDocPath: '',
    description: 'Detects unsafe HTTP data transmission that may expose sensitive information.'
};

export class UnsafeHttpTransmissionCheck implements BaseChecker {
                ...
                for (const stmt of cfg.getStmts()) {
                    if (stmt.getExprs().length > 0) {
                        ...
                        if (expr instanceof ArkStaticInvokeExpr && 
                            expr.getMethodSignature().getMethodSubSignature().getMethodName() == "request") {
                            // 检查是否是http模块的request方法
                            const declaringClass = expr.getMethodSignature().getDeclaringClassSignature().getClassName();
                            if (declaringClass.includes("http")) {
                                this.reportIssue(targetFile, stmt, "request");
                            }
                        }
                    }
                    
                    // 检查语句的原始文本是否包含HTTP相关的不安全模式
                    const text = stmt.getOriginalText();
                    if (text && (text.includes('http.request') || text.includes('hostname:') && text.includes('port: 80'))) {
                        this.reportIssue(targetFile, stmt, "unsafe-http");
                    }
                }
            }
        }
